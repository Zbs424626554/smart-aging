...
export default function Chat() {
  ...
  const autoCallOnceRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    console.log("[Elderly Chat] mount, userInfo=", !!userInfo);
    if (userInfo) {
      try {
        const user = JSON.parse(userInfo);
        console.log("[Elderly Chat] parsed user:", user?.username, user?.role);
        setCurrentUser({ username: user.username, role: user.role });

        // 异步连接WebSocket，不阻塞其他功能
        setTimeout(() => {
          console.log("[Elderly Chat] connecting WS for", user.username);
      ...
      try {
        ...
        if (token) {
          ...
          if (username) {
            console.log("[Elderly Chat] fallback from token:", username, role);
            setCurrentUser({ username, role });
            setTimeout(() => connectWebSocket(username), 100);
          }
        }
      } catch (e) {
        console.warn("[Elderly Chat] token decode failed", e);
      }
    }
  ...
  const handleCallResponse = async (data: any) => {
    if (data.conversationId === id) {
      if (data.response === "accept") {
        ...
        try {
          console.log('[RTC] handleCallResponse accept -> initializeWebRTCCall, responder=', data.responder, 'cid=', data.conversationId);
          const overrideReceivers = data.responder ? [data.responder] : undefined;
          await initializeWebRTCCall(overrideReceivers);
          console.log('[RTC] initializeWebRTCCall finished (offer created & sent)');
        } catch (error) {
  ...
  // 初始化WebRTC通话（发起方）
  const initializeWebRTCCall = async (overrideReceivers?: string[]) => {
    if (!currentUser || !id) return;

    try {
      console.log('[RTC] init start, overrideReceivers=', overrideReceivers, 'cid=', id, 'user=', currentUser?.username);
      // 根据通话类型选择本地媒体
      ...
      console.log('[RTC] created PC for offer');
      ...
      console.log('[RTC] offer created & setLocalDescription');

      // 发送Offer
      let receivers: string[] | undefined = overrideReceivers;
      if (!receivers) {
        if (!conversation) {
          console.warn('[RTC] initialize skipped: conversation not ready and no override receivers');
          return;
        }
      ...
      console.log('[WS] send webrtc_offer ->', receivers);
  ...
  const getAudioStream = async () => {
    try {
      ...
      const constraints: MediaStreamConstraints = { audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true }, video: false };
  ...
  // 若通过 URL 带入 ?call=1，且对话、WebSocket 就绪后自动发起语音通话（仅一次）
  useEffect(() => {
    if (
      shouldAutoCall &&
      isWebSocketConnected &&
      conversation &&
      !callSession.isActive &&
      !autoCallOnceRef.current
    ) {
      autoCallOnceRef.current = true;
      void handleVoiceCall();
    }
  }, [shouldAutoCall, isWebSocketConnected, conversation, callSession.isActive]);

  // 发起方超时未接处理：呼叫状态 60 秒自动结束
  useEffect(() => {
    if (callSession.isActive && callSession.status === "calling") {

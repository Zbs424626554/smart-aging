...
export class HealthController {
  ...
  static async getLatestHealthData(req: Request, res: Response) {
    console.log("getLatestHealthData", req.params);
    try {
      ...
      // 获取最新健康数据
      const latestHealthData = await HealthData.findOne({ elderlyId }).sort({
        lastUpdate: -1,
      });
  ...
  static async getHealthStats(req: Request, res: Response) {
    try {
      ...
      // 计算平均值
      const totalHeartRate = healthData.reduce(
        (sum, data) => sum + data.heartRate,
        0
      );
      const totalTemperature = healthData.reduce(
        (sum, data) => sum + data.temperature,
        0
      );
      const totalOxygenLevel = healthData.reduce(
        (sum, data) => sum + data.oxygenLevel,
        0
      );
      const totalBloodSugar = healthData.reduce(
        (sum, data) => sum + data.bloodSugar,
        0
      );

      const averageHeartRate = Math.round(totalHeartRate / healthData.length);
      const averageTemperature =
        Math.round((totalTemperature / healthData.length) * 10) / 10;
      const averageOxygenLevel = Math.round(
        totalOxygenLevel / healthData.length
      );
      const averageBloodSugar =
        Math.round((totalBloodSugar / healthData.length) * 10) / 10;
      ...
      const trends = {
        ...
        dates: healthData.map(
          (data) => data.lastUpdate.toISOString().split("T")[0]
        ),
      };
  ...
  static async getHealthWarnings(req: Request, res: Response) {
    try {
      ...
      // 检查血糖
      if (
        latestHealthData.bloodSugar < 3.9 ||
        latestHealthData.bloodSugar > 6.1
      ) {
        warnings.push({
          type: "bloodSugar",
          level: "warning",
